...
const SearchUnsplashed = ({setSearch}) => {
    ...
    const handleButtonSearch = () => {
      const url =`https://api.unsplash.com/search/photos?page=5&per_page=30&query=${searchValue}&client_id=${import.meta.env.VITE_ACCESS_KEY}`
      setSearch(false)
  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 my-10 max-w-7xl mx-auto">
      ...
      {result.map((photo)=>(
        <div key={photo.id}>
          <a href={photo.urls.small} target="_blank" rel="noreferrer" className="relative">
            <img className='w-full h-full max-h-[500px] object-cover' src={photo.urls.small} />
            {/* Download icon */}
